...
class IrActionsReport(models.Model):
    ...
    def _render_composer_mode(self, doc_template, doc_obj, data, context, autoescape=False):
        for idx, obj in enumerate(doc_obj):
            # Pre-process HTML fields to render as rich subdocument
            note_subdoc = None
            if hasattr(obj, 'note') and obj.note:
                try:
                    # Clean HTML to remove problematic tags for htmldocx
                    soup = BeautifulSoup(obj.note, 'html.parser')

                    # Remove problematic tags that cause htmldocx to fail
                    ...
                    # Convert HTML to Word subdocument
                    note_subdoc = misc_tools.render_html_as_subdoc(doc_template, cleaned_html)
                except Exception as e:
                    # Fallback: use empty subdoc if conversion fails
                    note_subdoc = None
            ...
            context = {
                **context,
                "docs": obj,
                "data": data,
                "note_subdoc": note_subdoc,
            }

            temp = BytesIO()
            # Listing objects handle escaping internally, so autoescape can be False
    ...
    def _get_libreoffice_path(self):
        libreoffice = self.env.ref('ormdoo_docx_report.default_libreoffice_path')
